...
(function () {
  ...
  function words(mount) {
    ...
    function docPanel() {
      var wrap = el('div');
      wrap.appendChild(C.subTitle('📚 文档词库（自定义添加 / 移除文档）'));
      var head = el('div', { class: 'row mb8' });
      head.appendChild(C.btn('➕ 添加文档', 'sm', function () { addDocFromFile(); }));
      head.appendChild(C.btn('✚ 新建空白文档', 'sm', function () { newDocManual(); }));
      wrap.appendChild(head);
      var docs = (s.wordDocs || []);
      if (!docs.length) { wrap.appendChild(el('div', { class: 'empty' }, '还没有文档，点「添加文档」导入 PDF / Word / txt')); return wrap; }
      var list = el('div', { class: 'doc-list' });
      docs.forEach(function (d) {
        var row = el('div', { class: 'doc-row' });
        var info = el('div', { class: 'doc-info' });
        info.appendChild(el('div', { class: 'doc-name' }, esc(d.name)));
        info.appendChild(el('div', { class: 'doc-meta' }, (d.type || 'txt').toUpperCase() + ' · ' + (d.words ? d.words.length : 0) + ' 词'));
        row.appendChild(info);
        var ops = el('div', { class: 'doc-ops' });
        ops.appendChild(C.btn('📥 导入今日', 'sm', function () {
        ...
        if (d.removable !== false) {
          ops.appendChild(C.btn('🗑', 'sm', function () {
            U.confirm('移除文档「' + d.name + '」？其单词也会从当日清单移除（生词本保留）').then(function (y) {
              if (y) { S.removeDoc(d.id); U.toast('已移除文档'); draw(); }
            });
          }));
        }
        row.appendChild(ops);
